...
def complex_field(ax, field, radius, vmax=None):
    with _circular_plot(ax, radius) as axes:
...
def field_relative_difference(ax, field, ref_field, radius, levels=None, scale=1, title=""):
    with _circular_plot(ax, radius, colorbar=False) as axes:
        ax_cartesian, ax_polar = axes
        xs, ys, mask = _cartesion_grid_circular_mask(radius, extension=0.1, number=500)
        zs = xs + 1j * ys
        field_values = ((field(zs).real/ref_field(zs).real)-1) * 10 ** scale
...
def plot_error_field_on_circle(field, ax, radius, levels=None, scale=1, title=""):
    with _circular_plot(ax, radius, colorbar=False) as axes:
        ax_cartesian, ax_polar = axes
        xs, ys, mask = _cartesion_grid_circular_mask(radius, extension=0.1, number=500)
